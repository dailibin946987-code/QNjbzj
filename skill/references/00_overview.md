...
| references/14_plain_descriptive_writing.md | 白描与反 AI 味协议（作者退场/名词动词干活） | 写稿/改稿每段自查；交付前随 04 扫描 |
...
## 创作引用顺序建议
创作任务：01 → 10 → 13(类型学) → 12 → 02 → 03 → 11（穿插）→ 写稿每段过 14(白描自查) → 交付前 04/08 监察。
（01 起流程 → 10 立结构 → 13 套类型学爆点语法 → 12 立人物 → 02 过审美 → 03 写台词 → 11 视听化与格式 → 14 白描去 AI 味自查 → 交付前 04/08 监察。）
审核任务：08 → 04（命中时）。
记忆/维护：05/06/07。
